...
一个自维护的 Vue 脚手架增量层。不替代 `create-vite`，而是在其产物之上，用脚本可重现地注入 Router / Pinia / Axios / UnoCSS，并通过 `sync` 持续追踪官方上游变更。解决的核心问题是：自定义模板如何在不 fork 官方仓库的前提下，保持可重现、可同步、可审计。
...
- **图标组件（可移除集成层的另一种方案）**：通过 `unplugin-icons` + `unplugin-vue-components` 可把图标当作 Vue 组件使用（如 `<i-carbon-home />`），数据源为 `@iconify/json`（全量图标集）。与上述原子类方案并存、各自独立可移除，详见下方「可移除集成层」
...
| `unplugin-auto-import` | 开发·集成层 | `vite.config.ts` 的 `AutoImport()` | 生成 `src/auto-imports.d.ts` | [unplugin-auto-import](https://github.com/unplugin/unplugin-auto-import) |
| `unplugin-vue-components` | 开发·集成层 | `vite.config.ts` 的 `Components()` | 生成 `src/components.d.ts` | [unplugin-vue-components](https://github.com/unplugin/unplugin-vue-components) |
| `unplugin-icons` | 开发·集成层 | `vite.config.ts` 的 `Icons()` + `IconsResolver` | `src/views/HomeView.vue`「图标组件」块 | [unplugin-icons](https://github.com/unplugin/unplugin-icons) |
| `@iconify/json` | 开发·集成层 | `unplugin-icons` 数据源 | 同上 | [Iconify JSON](https://iconify.design/docs/develop/json.html) |
...
## 可移除集成层（unplugin 系列，默认包含）

> 以下 4 个 `unplugin` 插件构成一层「默认包含的可移除层」，均已在 `vite.config.ts` 中以独立注释块配置，可整体或单独移除（遵循规则 7）。
...
> 依赖清单的单一事实来源为 `scripts/manifest.mjs` 的 `CUSTOM_DEPS`，`generate.mjs` 从此处驱动安装命令。以下手动命令与脚本完全对应。
...
| `pnpm add -D unplugin-...` | 可移除集成层：图标组件 + API/组件自动导入（详见「可移除集成层」） |
...
**5.2.1 复制可移除集成层的类型声明**（`vue-tsc` 先于 Vite 运行，需预先存在）
...
- 用途：可移除集成层（unplugin 系列）生成的全局类型声明，供 `vue-tsc` 类型检查自动导入的 API 与图标组件。详见「可移除集成层 → 生成的类型声明文件」。
...
> 依赖清单的单一事实来源为 `scripts/manifest.mjs` 的 `CUSTOM_DEPS`，以下命令与该清单对应。
...
- **用途**：
  ...
  - 第二行（可移除集成层）：`unplugin-icons` 图标按需编译为组件；`unplugin-auto-import` API 自动导入；`@iconify/json` 全量图标数据源；`unplugin-vue-components` 组件自动导入。详见「可移除集成层」。
...
- **用途**：注册 `UnoCSS()`（原子类）；可移除集成层注册 `AutoImport` / `Components` / `Icons`，启用 API 自动导入与图标组件。每块独立注释，可单独移除。
...
<template>
  <section class="m-4 p-4 border border-gray-300 rounded-lg">
    ...
    <!-- 图标组件（unplugin-icons + unplugin-vue-components）：依赖 @iconify/json，移除可移除集成层时删除此块即可 -->
    <div class="mt-4 flex items-center gap-3 text-xl">
      <i-carbon-home class="text-green-600" />
      <i-carbon-information class="text-blue-600" />
...
```bash
npx vite build  # 首次：先生成可移除集成层的 .d.ts（auto-imports.d.ts / components.d.ts）
pnpm build      # 类型检查 + 生产构建
pnpm dev        # 启动开发服务器
pnpm preview    # 预览构建产物
```

- **用途**：`pnpm build` 即 `vue-tsc -b && vite build`，先类型检查再构建。因 `vue-tsc` 先于 Vite 运行，可移除集成层（第 5 步）生成的 `.d.ts` 需预先存在，故首次先跑一次 `npx vite build`（或 `pnpm dev`）生成它们。`pnpm dev` 启动 HMR 开发服务器；`pnpm preview` 本地预览 `dist/`。
...
├── vite.config.ts          # 官方（脚本加 UnoCSS + 可移除集成层 unplugin 插件）
...
│   ├── assets/             # 官方，不改
│   ├── auto-imports.d.ts   # 新增（可移除集成层生成）：自动导入 API 类型声明
│   ├── components.d.ts     # 新增（可移除集成层生成）：自动解析组件类型声明
...
- 执行 `pnpm generate` 时，交互 `Install with pnpm and start now?` 选择 `No` 使脚本自动继续执行。