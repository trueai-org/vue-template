...
1. **不过度封装** - 无 bootstrap、无 http 解包封装、无 UI 组件封装
...
| `hooks`   | 可选 | husky · lint-staged · @commitlint/cli · @commitlint/config-conventional          | `.husky/` · `commitlint.config.js` · `.lintstagedrc.json` | [Husky](https://typicode.github.io/husky) · [lint-staged](https://github.com/lint-staged/lint-staged) · [commitlint](https://commitlint.js.org)                                                    |
...
可选模块（回车跳过，y 安装）:
  ? 安装 hooks - Git 钩子（Husky + lint-staged + commitlint）(y/N)
